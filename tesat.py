...
# ----------------------------
# 3. POLL SESSION FOR PR
# ----------------------------
def wait_for_pr(session_id, timeout=900):
    logger.info(f"Starting to poll session {session_id} for PR (timeout: {timeout}s)")
    start = time.time()
    ...
    while True:
        try:
            ...
            logger.info(f"Session poll status: {response.status_code}")
            
            if response.status_code != 200:
                logger.error(f"Session poll failed: {response.text}")
            ...
            if time.time() - start > timeout:
                logger.error(f"Timeout waiting for PR after {timeout} seconds")
                raise TimeoutError("Timeout waiting for PR")
            
            logger.info(f"Waiting 10 seconds before next poll...")
            time.sleep(10)
            
        except requests.exceptions.HTTPError as e:
            logger.error(f"HTTP Error polling session: {e}")
            logger.error(f"Response status: {e.response.status_code}")
            logger.error(f"Response text: {e.response.text}")
            logger.error(f"Request URL: {e.response.url}")
            raise
        except Exception as e:
            logger.error(f"Unexpected error polling session: {e}")
            raise
...
def archive_session(session_id):
    ...
    try:
        response = requests.post(
            f"{BASE_URL}/sessions/{session_id}/archive",
            headers=HEADERS
        )
        
        logger.info(f"Archive response status: {response.status_code}")
        logger.info(f"Archive response text: {response.text}")
        
        if response.status_code in [200, 204]:
            logger.info(f"Session archived successfully: {session_id}")
        else:
            logger.warning(f"Archive failed with status {response.status_code}: {response.text}")
...
def run():
    ...
    try:
        ...
        final_status = monitor_session_status(session_id)

        # Use the list-based monitoring that works
